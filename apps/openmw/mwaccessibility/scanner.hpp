...
namespace MWAccessibility
{
    ...
    private:
        ...
        struct Waypoint
        {
            ...
            // A destination route. Activate and Shift+Enter execute this same
            // immutable path; facing and the beacon point to its road entrance.
            bool mIsRoad = false;
            std::shared_ptr<const RoadRoute> mRoadRoute;
        };
        ...
        // --- Road following ---------------------------------------------
        // Plan globally from immutable terrain records, execute locally with
        // the existing walker and all its safety/recovery behavior. The route
        // being walked is the one selected, not a fresh greedy choice per tile.
        mutable std::unique_ptr<RoadRoutePlanner> mRoadPlanner;
        mutable std::optional<RoadTile> mRoadRoutesAt;
        mutable std::string mRoadRoutesOrigin;
        mutable std::vector<std::shared_ptr<const RoadRoute>> mAvailableRoadRoutes;
        bool mFollowingRoad = false;
        std::shared_ptr<const RoadRoute> mActiveRoadRoute;
        std::size_t mRoadStep = 0;
        ...
        void startFollowingRoad(std::shared_ptr<const RoadRoute> route);
        bool startRoadLeg();
